...
enum UARTRxMode ut_serial::UTSerialRx(uint8_t byte, uint8_t * data_buffer, function<void (uint8_t, uint8_t, const uint8_t *)> handler)
{
    ...
    *rx_ptr++ = byte;
    ...
    {
    if (byte == crc.bytes[1]) handler(component_id, message_id, (const uint8_t *)&data_buffer);
    goto RESET;
    }
    bytes_processed++;
...
bool ut_serial::recv_data(function<void (uint8_t, uint8_t, const uint8_t *)> handler)
{
	static int flag = 0;
    ...
    while (serial.Read(&rx_byte, 1))
	{
	  switch (rx_mode)
	  {
		...
		case UART_RX_MODE_UT_ONGOING:
		  rx_mode = UTSerialRx(rx_byte, data_buffer, handler);
          flag = 1;
		  break;
	  }
	}
